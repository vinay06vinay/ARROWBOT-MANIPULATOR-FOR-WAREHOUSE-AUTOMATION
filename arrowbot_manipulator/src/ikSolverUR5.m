function q = ikSolverUR5(pos,eul,qPrevious)
...
for i = 1:length(theta1_)
    ...
    if acosValue > 1 %If this happens all the time, maybe just set theta5 to zero.
        error('Theta5 can not be determined. Value inside acos is above 1 and the solution is therefore not valied.')
    end
    for sign = [1 -1]
